...
import '../../../AdminUi/AdminReports/constants.dart';
import '../EmployeeConstants.dart';
import 'EmployeeDailyReportsPdf.dart';


class EmpPresentReport extends StatefulWidget {
  const EmpPresentReport({Key? key});

  @override
  State<EmpPresentReport> createState() => _EmpPresentReportState();
}

class _EmpPresentReportState extends State<EmpPresentReport> {
  ...
  Future<void> _createPdf(List<Map<String, dynamic>> data) async {
    ...
    final ByteData byteData = await rootBundle.load('assets/images/pioneer_logo_app.png');
    final Uint8List imageBytes = byteData.buffer.asUint8List();
    final PdfImage logoImage = PdfBitmap(imageBytes);
    page.graphics.drawImage(
      logoImage,
      Rect.fromLTWH(50, logoY+100, 100, 50), // Adjust position and size as needed
    );

    // Save and launch the PDF
    final List<int> bytes = await document.save();
    document.dispose();
    empSaveAndLaunchFile(bytes, "present_reports.pdf");
  }
  ...
  Future<void> _createExcel(List<Map<String, dynamic>> data) async {
    ...
    final String fileName = '$path/present_report.xlsx';
            ...
            child: DataTable(
              ...
              rows: filteredData.map((data) {
                return DataRow(cells: [
                  ...
                  DataCell(Text(data['Working Hours'])),
                ]);
              }).toList(),
            ),
          ),
